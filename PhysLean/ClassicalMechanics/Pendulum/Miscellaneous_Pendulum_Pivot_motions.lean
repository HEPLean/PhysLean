...
In all three cases described below, the point of support moves in the same plane as the pendulum 
according to a given function of time. The lagrangian of the pendulum is to be found.
...
x = a cos(γ t) + l sin(φ)
y = a sin(γ t) - l cos(φ)       
(where generalized coordinate, φ, is the angle the string makes with vertical, γ is angle with horizontal)
...
Likewise, m g a sin(γ t) in V is ignored as its contribution to action  is constant
...
V = m g y = - m g l cos(φ)
We ignore the constant term 1/2 m a^2 γ^2 sin^2(γ t) in T as it does not contribute to variation.
...
x = l sin(φ)
y = a cos(γ t) - l cos(φ)       
(where generalized coordinate, φ, is the angle the string makes with vertical)
L = T - V where
T = 1/2 m (x_dot^2 + y_dot^2) 
