...
In all three cases described below, the point of support moves in the same plane as the pendulum according
to a given function of time. The lagrangian of the pendulum is to be found.
...
x = a cos(γ t) + l sin(φ)
y = a sin(γ t) - l cos(φ)       (where generalized coordinate, φ, is the angle the string makes with vertical, γ is angle with horizontal)
...
Like wise, m g a sin(γ t) in V can be ignored since its contribution to the action function is constant
...
Since lagrangians differing by a total time derivate lead to the same equations of motion, we can ignore the last term.
So that the final lagrangian becomes:
...
L = T - V where
T = 1/2 m (x_dot^2 + y_dot^2) = 1/2 m [l^2 φ_dot^2 + a^2 γ^2 sin^2(γ t) - 2 a l γ sin(γ t) φ_dot cos(φ)]
V = m g y = - m g l cos(φ)
We can ignore the constant term 1/2 m a^2 γ^2 sin^2(γ t) in T as it does not affect the equations of motion.
...
The coordinates of m can be expressed as:
x = l sin(φ)
y = a cos(γ t) - l cos(φ)       (where generalized coordinate, φ, is the angle the string makes with vertical)
L = T - V where
T = 1/2 m (x_dot^2 + y_dot^2) = 1/2 m [l^2 φ_dot^2 + a^2 γ^2 sin^2(γ t) - 2 a l γ sin(γ t) φ_dot sin(φ)]
V = m g y = m g [a cos(γ t) - l cos(φ)]
We can ignore the constant term 1/2 m a^2 γ^2 sin^2(γ t) in T as it does not affect the equations of motion.
Likewise, m g a cos(γ t) in V can be ignored since its contribution to the action function is constant
