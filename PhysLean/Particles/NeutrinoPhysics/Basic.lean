...
lemma diagPhase_star (θ : Fin 3 → ℝ) :
  ...
  ext i j
  fin_cases i <;> fin_cases j <;> simp [diagPhase, Matrix.conjTranspose_apply, ← exp_conj]
...
lemma diagPhase_mul (θ φ : Fin 3 → ℝ) :
  ...
  ext i j
  fin_cases i <;> fin_cases j <;> simp [diagPhase, Matrix.mul_apply, ← exp_add, mul_add]
...
def diagPhase_unitary (θ : Fin 3 → ℝ) : unitaryGroup (Fin 3) ℂ :=
  ...
  ext i j
  fin_cases i <;> fin_cases j <;> simp [diagPhase, Matrix.mul_apply, Matrix.conjTranspose_apply, ← exp_conj, ← exp_add]
  ⟩
