...
as such we define the type `TimeMetric` (equivalent to the positive reals) to
...
and we will use the phrase 'units of time' interchangeably 'term of TimeMetric'.

Given a `x : TimeMetric`, and the structure above, we can define the following
...
def TimeMetric : Type := {x : ℝ | 0 < x}

namespace TimeMetric

@[simp]
lemma val_neq_zero (x : TimeMetric) : x.val ≠ 0 := by
...
## Functions based on a choice of metric

-/

namespace TimeTransMan

/-- The distance between two points in `TimeTransMan` in the units of `x : TimeMetric`. -/
noncomputable def dist (x : TimeMetric) (t1 t2 : TimeTransMan) : ℝ :=
...
/-- The signed difference between two points in on the manifold `TimeTransMan`
  in the units of `x : TimeMetric`. -/
noncomputable def diff (x : TimeMetric) (t2 t1 : TimeTransMan) : ℝ :=
  if t1 ≤ t2 then dist x t1 t2 else - dist x t1 t2

lemma diff_eq_val (x : TimeMetric) (t1 t2 : TimeTransMan) :
...
@[simp]
lemma diff_self (x : TimeMetric) (t : TimeTransMan) :
    diff x t t = 0 := by
  simp [diff_eq_val]

lemma diff_fst_injective (x : TimeMetric) (t : TimeTransMan) : Function.Injective (diff x · t) := by
  intro t1 t2 h
...
lemma diff_fst_surjective (x : TimeMetric) (t : TimeTransMan) :
...
lemma diff_fst_bijective (x : TimeMetric) (t : TimeTransMan) :
...
/-- Given a time unit `x : TimeMetric`, `addTime x r t`, for a real `ℝ` and `t : TimeTransMan`,
  is the point in `TimeTransMan` seprated from `t` by a difference of `r` in the units `x`.
  For example, if `x` corresponds to seconds `addTime x 1 t` is the time 1 second more then `t`. -/
noncomputable def addTime (x : TimeMetric) (r : ℝ) (t : TimeTransMan) : TimeTransMan :=
  Function.invFun (diff x · t) r

lemma addTime_eq_val (x : TimeMetric) (r : ℝ) (t : TimeTransMan) :
...
lemma addTime_val (x : TimeMetric) (r : ℝ) (t : TimeTransMan) :
...
@[simp]
lemma toTime_symm_zero_add (zero : TimeTransMan) (x : TimeMetric) :
    (toTime zero x).symm 0 = zero := by
  ext
  simp [toTime, addTime_val, diff_eq_val]

lemma toTime_val (zero : TimeTransMan) (x : TimeMetric) (t : TimeTransMan) :
    (toTime zero x t).val = diff x t zero := by rfl

lemma toTime_symm_val (zero : TimeTransMan) (x : TimeMetric) (r : Time) :
    (toTime zero x).symm r = addTime x r zero := by
  ext
  simp [toTime, addTime_val, diff_eq_val]

@[simp]
lemma toTime_addTime (zero : TimeTransMan) (x : TimeMetric) (r : ℝ) (τ : TimeTransMan) :
...
lemma toTime_symm_add (zero : TimeTransMan) (x : TimeMetric) (t1 t2 : Time) :
...
lemma diff_eq_toTime_sub (zero : TimeTransMan) (x : TimeMetric) (t1 t2 : TimeTransMan) :
