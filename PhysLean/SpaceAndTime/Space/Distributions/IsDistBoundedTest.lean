...
structure IsDistBounded {d : ℕ} (f : Space d → F) : Prop where
  integrable' : ∀ (η : 𝓢(Time × Space d, ℝ)), Integrable (fun x => η x • f x.2)
...
@[fun_prop]
lemma integrable {d : ℕ} {f : Space d → F} (hf : IsDistBounded f)
    (η : 𝓢(Space d, ℝ)) : Integrable (fun x => η x • f x) :=
  hf.integrable' η

lemma integrable_time_schwartzMap_smul {d : ℕ} {f : Space d → F}
    (hf : IsDistBounded f) (η : 𝓢(Time × Space d, ℝ)) :
    Integrable (fun x => η x • f x.2) := by
  have h1 : Integrable (fun x => ∫ (t : Time), η (t, x) • f x) := by
    sorry
  have h2 : Integrable (fun x => ∫ (t : Time), ‖η (t, x) • f x‖) := by
    sorry
...
## B. Basic properties of IsDistBounded
...
@[fun_prop]
lemma IsDistBounded.add {d : ℕ} {f g : Space d → F}
...
lemma IsDistBounded.mono {d : ℕ} {f : Space d → F} {g : Space d → F'}
    (hf : IsDistBounded f) (hgae : AEStronglyMeasurable g volume)
    (hfae : AEStronglyMeasurable f volume) (hfg : ∀ x, ‖g x‖ ≤ ‖f x‖) : IsDistBounded g := by
  intro η
  specialize hf η
  rw [← MeasureTheory.integrable_norm_iff] at hf ⊢
  apply Integrable.mono' (g := fun x => ‖η x‖ * ‖f x‖) _
  · fun_prop
  · filter_upwards with x
    simp
    simp [norm_smul]
    refine mul_le_mul_of_nonneg (by rfl) (hfg x) (by positivity) (by positivity)
  · convert hf using 1
    funext x
    simp [norm_smul]
  · fun_prop
...
@[fun_prop]
lemma IsDistBounded.mul_const_fun {dm1 : ℕ}
    {f : EuclideanSpace ℝ (Fin dm1.succ) → ℝ}
    (hf : IsDistBounded f) (c : ℝ) : IsDistBounded (fun x => f x * c) := by
  convert hf.const_mul_fun c using 1
  funext x
  ring
...
@[fun_prop]
lemma IsDistBounded.zero {dm1}  : IsDistBounded (0 : EuclideanSpace ℝ (Fin (dm1 + 1)) → F) := by
  use 1, fun _ => 0, fun _ => 0, fun _ => 0
  simp

@[fun_prop]
lemma IsDistBounded.sum {ι : Type*} {s : Finset ι} {dm1 : ℕ}
    {f : ι → EuclideanSpace ℝ (Fin dm1.succ) → F}
...
@[fun_prop]
lemma IsDistBounded.sum_fun {ι : Type*} {s : Finset ι} {dm1 : ℕ}
    {f : ι → EuclideanSpace ℝ (Fin dm1.succ) → F}
...
@[fun_prop]
lemma IsDistBounded.nat_pow {dm1 : ℕ} (n : ℕ) :
    IsDistBounded (dm1 := dm1) (fun x => ‖x‖ ^ n) := by
  refine ⟨1, fun _ => 1, fun _ => 0, fun _ => n, ?_, ?_, ?_⟩
  · simp
  · simp
...
lemma IsDistBounded.nat_pow_add_const {dm1 : ℕ} (n : ℕ) (a : ℝ) :
    IsDistBounded (dm1 := dm1) (fun x => (‖x‖ + a) ^ n) := by
  conv =>
    enter [1, x]
    rw [add_pow]
...
lemma IsDistBounded.log {n : ℕ} :
    IsDistBounded (dm1 := n.succ) (fun x => Real.log ‖x‖) := by
  apply IsDistBounded.mono (f := fun x => ‖x‖⁻¹ + ‖x‖)
  · apply IsDistBounded.add
    · apply IsDistBounded.inv
    · convert IsDistBounded.pow (dm1 := n.succ) 1 (by simp; omega)
      simp
  · intro x
    simp
