...
lemma slice_symm_measurableEmbedding {d : ℕ} (i : Fin d.succ) :
  ...
  change MeasurableEmbedding (fun (p : ℝ × Space d) j =>
...
lemma schwartzMap_slice_bound {n m} {d : ℕ} (i : Fin d.succ) :
    ...
    Integrable (fun x : ℝ => ‖((1 + ‖x‖) ^ rt)⁻¹‖) volume ∧
     (∀ (x : Space d), ∀ r, ‖(slice i).symm (r, x)‖ ^ m * ‖iteratedFDeriv ℝ n ⇑η ((slice i).symm (r, x))‖ ≤
        k * ‖(1 + ‖r‖) ^ (rt)‖⁻¹)
...
@[fun_prop]
lemma schwartzMap_iteratedFDeriv_norm_slice_symm_integrable {n} {d : ℕ} (η : 𝓢(Space d.succ, ℝ)) (x : Space d)
    (i : Fin d.succ) :
...
@[fun_prop]
lemma schwartzMap_iteratedFDeriv_slice_symm_integrable {n} {d : ℕ} (η : 𝓢(Space d.succ, ℝ)) (x : Space d)
    (i : Fin d.succ) :
    Integrable (fun r => iteratedFDeriv ℝ n ⇑η (((slice i).symm (r, x)))) volume := by
...
lemma schwartzMap_slice_integral_hasFDerivAt {d : ℕ} (η : 𝓢(Space d.succ, ℝ)) (i : Fin d.succ) (x₀ : Space d) :
  ...
  apply hasFDerivAt_integral_of_dominated_of_fderiv_le
    (bound := fun t => (k * ‖(slice i).symm.toContinuousLinearMap.comp
          (ContinuousLinearMap.prod (0 : Space d →L[ℝ] ℝ ) (ContinuousLinearMap.id ℝ (Space d)))‖) * ‖(1 + ‖t‖) ^ (rt)‖⁻¹) (ε := 1)
  · simp
  ...
  | succ n ih =>
    ...
    · rw [contDiff_clm_apply_iff]
      ...
      have hl2 : (fun x => ∫ (r : ℝ), (fderiv ℝ (fun x => η (((slice i).symm (r, x)))) x) y)=
          fun x => ∫ (r : ℝ), SchwartzMap.pderivCLM ℝ ((slice i).symm (0, y))  η (((slice i).symm (r, x))) := by
        funext x
        congr
        funext t
...
lemma schwartzMap_slice_integral_iteratedFDeriv_apply {d : ℕ} (n : ℕ) (η : 𝓢(Space d.succ, ℝ)) (i : Fin d.succ) :
  ...
  | succ n ih =>
    ...
    calc _
      ...
      _ = ∫ (r : ℝ), (fderiv ℝ (fun x => ((iteratedPDeriv ℝ fun j => (slice i).symm (0, Fin.tail y j)) η)
...
lemma schwartzMap_slice_integral_iteratedFDeriv {d : ℕ} (n : ℕ) (η : 𝓢(Space d.succ, ℝ)) (i : Fin d.succ)
    (x : Space d) :
...
lemma schwartzMap_slice_integral_iteratedFDeriv_norm_le {d : ℕ} (n : ℕ) (η : 𝓢(Space d.succ, ℝ)) (i : Fin d.succ)
    (x : Space d) :
    ‖iteratedFDeriv ℝ n (fun x => ∫ (r : ℝ), η ((slice i).symm (r, x))) x‖ ≤
        (∫ (r : ℝ), ‖iteratedFDeriv ℝ n η ((slice i).symm (r, x))‖) *
        ‖(slice i).symm.toContinuousLinearMap.comp
          (ContinuousLinearMap.prod (0 : Space d →L[ℝ] ℝ ) (ContinuousLinearMap.id ℝ (Space d)))‖ ^ n := by
...
lemma schwartzMap_mul_pow_slice_integral_iteratedFDeriv_norm_le {d : ℕ} (n m : ℕ) (i : Fin d.succ) :
    ...
    ‖x‖ ^ m * ‖iteratedFDeriv ℝ n (fun x => ∫ (r : ℝ), η ((slice i).symm (r, x))) x‖ ≤
        ((∫ (r : ℝ), ‖((1 + ‖r‖) ^ rt)⁻¹‖) *
        ‖(slice i).symm.toContinuousLinearMap.comp
          (ContinuousLinearMap.prod (0 : Space d →L[ℝ] ℝ ) (ContinuousLinearMap.id ℝ (Space d)))‖ ^ n )
  ...
  calc _
      ...
      _ ≤ (∫ (r : ℝ), ‖((slice i).symm (r, x))‖ ^ m * ‖iteratedFDeriv ℝ n η (((slice i).symm (r, x)))‖) *
          ‖(slice i).symm.toContinuousLinearMap.comp
          ((0 : Space d →L[ℝ] ℝ ).prod (ContinuousLinearMap.id ℝ (Space d)))‖ ^ n := by
...
def sliceSchwartz {d : ℕ} (i : Fin d.succ) :
  ...
  · simp
    intro m n
    obtain ⟨rt, hrt⟩ := schwartzMap_mul_pow_slice_integral_iteratedFDeriv_norm_le (d := d) (n := n) (m := m) i
