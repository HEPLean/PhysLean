...
This module  the `d`-dimensional hydrogen atom with `1/r` potential.

In addition to `d`, the quantum mechanical system is characterized by a mass `m > 0`
and constant `k` appearing in the potential `V = -k/r`. The standard hydrogen atom has
`d=3`, `m = mₑmₚ/(mₑ + mₚ) ≈ mₑ` and `k = e²/4πε₀`.
...
/-- The hydrogen atom Hamiltonian regularized by `ε` is defined to be
  `𝐩²/(2m) - k(𝐫⁻¹ + ½ε²𝐫⁻³)`, where `𝐫ˢ` is the regularized inverse radius
  operator which maps `ψ : 𝓢(Space d, ℂ)` to `(‖x‖² + ε²)^(s/2) • ψ`. -/
