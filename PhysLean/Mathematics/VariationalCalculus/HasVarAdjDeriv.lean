...
lemma prod [OpensMeasurableSpace X] [IsFiniteMeasureOnCompacts (volume (α := X))] {F : (X → U) → (X → V)} {G : (X → U) → (X → W)} {F' G'}
  ...
  linearize := by
    ...
    · rw [← fderiv_deriv, ← fderiv_deriv, DifferentiableAt.fderiv_prodMk,
      ...
      · apply ContDiff.differentiable (n := ∞) _ (by simp)
        exact smooth_R hG hφ x
  adjoint := by
    apply HasVarAdjoint.congr_fun (G := fun δu x => (deriv (fun s => F (fun x' => u x' + s • δu x') x) (0 : ℝ),
...
lemma fst {F : (X → U) → (X → W×V)}
  ...
  adjoint := by
    apply HasVarAdjoint.congr_fun (G := (fun δu x => (deriv (fun s => (F (fun x' => u x' + s • δu x') x)) (0 :ℝ)).1) )
...
lemma snd {F : (X → U) → (X → W×V)}
  ...
  linearize := by
    ...
    · apply ContDiff.differentiable (n := ∞) (hF.smooth_R hφ x) (by simp)
  adjoint := by
    apply HasVarAdjoint.congr_fun (G := (fun δu x => (deriv (fun s => (F (fun x' => u x' + s • δu x') x)) (0 :ℝ)).2) )
...
lemma deriv' (u : ℝ → U) (hu : ContDiff ℝ ∞ u) :
  ...
  adjoint := by
    ...
    · intro φ hφ
      ...
      conv_lhs =>
        enter [1, x]
        rw [deriv_const_smul _ (by fun_prop)]
      rw [deriv_smul_const]
      simp
      fun_prop
...
protected lemma gradient {d} (u : Space d → ℝ) (hu : ContDiff ℝ ∞ u) :
  ...
  apply hasVarAdjDerivAt_of_hasVarAdjoint_of_linear
  · intro φ hφ
    simp [← Space.grad_eq_gradiant, Space.grad_eq_sum]
    apply ContDiff.sum
    intro i _
    apply ContDiff.smul
    simp [Space.deriv]
  ...
  · intro φ hφ x
    rw [← Space.grad_eq_gradiant, Space.grad_eq_sum]
    conv_lhs => enter [1, x]; rw [← Space.grad_eq_gradiant, Space.grad_eq_sum]
    rw [deriv_sum]
    congr
    funext i
    rw [deriv_smul_const]
